...
from bonus_intent_cls_utils import my_argparse , load_config , get_dataloader , print_progress
from bonus_intent_cls_data import load_json

transformers.utils.logging.set_verbosity(transformers.logging.ERROR)

def train(train_data , validation_data , config , intent2label , tokenizer , model , device):
	train_dataloader = get_dataloader(train_data , tokenizer , intent2label , config , 'train')
	model.to(device)
	...
	for i in range(config['epoch']):
		...
		start = time()
		for j , (input_ids , token_type_ids , attention_mask , labels) in enumerate(train_dataloader):
			...
			count += torch.sum(labels == index).item()
		...
		accuracy = evaluate(validation_data , config , intent2label , tokenizer , model , device)
		if accuracy >= max_accuracy:
			print('save model...')
			model.save_pretrained(os.path.join(config['checkpoint_root'] , config['checkpoint_name']))
			max_accuracy = accuracy

def evaluate(validation_data , config , intent2label , tokenizer , model , device):
	validation_dataloader = get_dataloader(validation_data , tokenizer , intent2label , config , 'validation')
	model.to(device)
	model.eval()
	count , total_loss = 0 , 0
	start = time()
	with torch.no_grad():
		for input_ids , token_type_ids , attention_mask , labels in validation_dataloader:
			input_ids , token_type_ids , attention_mask , labels = input_ids.to(device) , token_type_ids.to(device) , attention_mask.to(device) , labels.to(device)
			output = model(input_ids = input_ids , token_type_ids = token_type_ids , attention_mask = attention_mask , labels = labels)
			logits , loss = output.logits , output.loss
			_ , index = torch.max(logits , dim = 1)
...
def main(args):
	...
	intent2label = {intent_cls_label[i]['intent'] : intent_cls_label[i]['label'] for i in range(len(intent_cls_label))}
	print('train model...')
	config = load_config()
	tokenizer = AutoTokenizer.from_pretrained(config['model_name'])
	model = AutoModelForSequenceClassification.from_pretrained(config['model_name'] , num_labels = len(intent_cls_label))
	train(train_data , validation_data , config , intent2label , tokenizer , model , device)
