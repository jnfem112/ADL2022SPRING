...
import transformers
from transformers import AutoConfig , AutoTokenizer , AutoModelForMultipleChoice , get_linear_schedule_with_warmup
...
def train(context , train_data , validation_data , config , tokenizer , model , device , plot):
	train_dataloader = get_dataloader(context , train_data , tokenizer , config , 'MultipleChoice' , 'train')
	model.to(device)
	...
	if plot:
		plot_learning_curve(train_loss_list , validation_loss_list , train_accuracy_list , validation_accuracy_list , 'MultipleChoice')

def evaluate(context , validation_data , config , tokenizer , model , device):
	validation_dataloader = get_dataloader(context , validation_data , tokenizer , config , 'MultipleChoice' , 'validation')
	model.to(device)
	model.eval()
	count , total_loss = 0 , 0
	start = time()
...
def main(args):
	...
	print('train model...')
	config , _ = load_config(args.base_model)
	tokenizer = AutoTokenizer.from_pretrained(config['model_name'])
	if args.pretrained:
		model = AutoModelForMultipleChoice.from_pretrained(config['model_name'])
	else:
		model_config = AutoConfig.from_pretrained(config['model_name'])
		model = AutoModelForMultipleChoice.from_config(model_config)
